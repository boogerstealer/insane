...
module.exports = class extends BaseCommand {
    constructor() {
        super({
            config: {
                name: 'config',
                description: 'Configure Tadu\'s settings for this server.',
    ...
    async run(client, message, args) {
        ...
        if (action === 'view') {
            ...
            switch (key) {
                case 'all': {
                    const yn = await client.awaitReply(message, 'Are you sure to reset Tadu\'s configurations for this server (yes/no)? _You have 15 seconds to comply_', 15000);
                    ...
                    Promise.all([await db.destroy(false, false)]).then(() => {
                        return message.channel.send(new Discord.MessageEmbed()
                            .setColor('RANDOM')
                            .setDescription(`Successfully reseted Tadu\'s configurations for this server.`)
                            .setTimestamp()
                ...
                case 'prefix': {
                    const yn = await client.awaitReply(message, `Are you sure to reset Tadu\'s \`${key}\` for this server (yes/no)? _You have 15 seconds to comply_`, 15000);
                    if (!['y', 'yes', 'YES'].includes(yn)) return message.channel.send('Request terminated.');
                    ...
                    Promise.all([await settings.save()]).then(() => {
                        return message.channel.send(new Discord.MessageEmbed()
                            .setColor('RANDOM')
                            .setDescription(`Successfully reseted Tadu\'s \`${key}\` for this server.`)
                            .setTimestamp()
                            .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }) || client.user.displayAvatarURL({ dynamic: true }))
                        );
                    }).catch(e => {
            ...
            switch (key) {
                ...
                case 'modLogChannel': {
                    ...
                    if (!channel.permissionsFor(message.guild.me).has(['SEND_MESSAGES'])) {
                        return message.channel.send(new Discord.MessageEmbed()
                            .setColor('RED')
                            .setDescription(`Error:Tadu doesn't have access to that channel`)
                            .setTimestamp()
                            .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }) || client.user.displayAvatarURL({ dynamic: true }))
                        );
                    }
